...
export default function AlbumCollapse({album, tracks}: {album: Album, tracks: SimplifiedTrackObject[]}) {
	...
	function expandAccordian() {
		albumTracks? setAlbumTracks(null):setAlbumTracks(() => tracks.map(x => [<Image key={setUniqueKey()} alt="tiny album cover" src={albumImage.url} height={tinyImageHeight} width={tinyImageWidth} />,<p className="col-span-8 tracking-wide" key={setUniqueKey()}>{x.name}</p>])); 
	}

	return (
		<div>
			<div className="flex gap-9 items-center tracking-wide">
