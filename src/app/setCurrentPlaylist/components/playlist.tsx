...
export default function Playlist({playlist, handleClick}: {playlist: SimplifiedPlaylistObject, handleClick: () => void}) {
	...
	return (
		<button className="font-tropiLand flex flex-col items-start gap-2 m-4 tracking-wider snap-start snap-always scroll-mt-4" onClick={handleClick}>
			<h2>{playlist.name}</h2>
			...
			<p className="text-textColor">{playlist.description}</p>
		</button>	
	);
}
